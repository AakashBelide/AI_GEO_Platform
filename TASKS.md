...
> **Redesigned** into a modern **dark dashboard** (Tailwind + **D3.js v7** via CDN): hero + stat tiles,
> interactive **dumbbell** gap chart, per-engine **CI dot-plots with error whiskers**, top-domain
> small-multiples, and a **D3 Jaccard heatmap** (color scale + legend). Trade-off: rendered page needs
> internet (CDN) — no longer a fully offline single file.
